	...
	<div class="blocked no-margin font-open-sans">
		...
		<div class="dev-row">
			...
			<div class="dev-col dev-col-6x">
				...
				<p class="dev-color-grey">if you having trouble with this service, please <a href="mailto:yitzak01@gmail.com" class="btn-link">ask for help</a>.</p>
				...
				<form action="{{ route('contact') }}" method="POST" class="margin-top-zx">
					...
					<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
						...
						<input type="text" name="name" id="name" class="dev-form-ctrl" required="required">
						{!! $errors->first('name', '<span class="help-block">:message</span>') !!}
					</div>
					<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
						<label for="email" class="label-ctrl">Email</label>
						<input type="email" name="email" id="email" class="dev-form-ctrl" required="required">
